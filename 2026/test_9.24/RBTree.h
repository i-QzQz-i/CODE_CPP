...
namespace QzQz
{
	...
	struct RBTreeNode
	{
		...
		Colour _col;
		//int col = 0
		RBTreeNode(const data& d, Colour col = Red)
			: _data(d)
			, _left(nullptr)
			, _right(nullptr)
			, _parent(nullptr)
	...
	struct RBTreeIterator
	{
		...
		Self& operator++()
		{
			...
			{
				/*while (cur && cur->_data < _node->_data)
				{
					cur = cur->_parent;
				}*/
		...
		Self& operator--()
		{
			...
			{
				/*while (cur && _node->_data < cur->_data)
				{
					cur = cur->_parent;
				}*/
	...
	public:
		...
		pair<Iterator, bool> Insert(const V& v)
		{
			...
			while (tmp)
			{
				if (tmp->_left != nullptr && KofT(v) < KofT(tmp->_data))
				{
					tmp = tmp->_left;
				}
				else if (tmp->_right != nullptr && KofT(tmp->_data) < KofT(v))
				{
					tmp = tmp->_right;
				}
				else
			...
			// 开始插入
			if (KofT(v) < KofT(tmp->_data))
			{
				tmp->_left = newnode;
			}
			else if (KofT(tmp->_data) < KofT(v))
			{
				tmp->_right = newnode;
			}
			else
			{
				return pair<Iterator, bool>(Iterator(ret, _root), false);
			}
			newnode->_parent = tmp;
		...
		Iterator Find(const K& key) const
		{
			...
			while (tmp)
			{
				if (less(key, KofT(tmp->_data)))
				{
					tmp = tmp->_left;
				}
				else if (less(KofT(tmp->_data), key))
				{
					tmp = tmp->_right;
				}
				else

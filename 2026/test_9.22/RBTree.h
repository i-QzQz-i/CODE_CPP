...
namespace QzQz
{
	...
	template<class date, class Ref, class Ptr>
	struct RBTreeIterator
	{
		typedef RBTreeNode<date> Node;
		typedef RBTreeIterator<date, Ref, Ptr> Self;
		...
		Self& operator++()
		{
			...
			{
				/*while (cur && cur->_date < _node->_date)
				{
					cur = cur->_parent;
				}*/
		...
		Self& operator--()
		{
			...
			{
				/*while (cur && _node->_date < cur->_date)
				{
					cur = cur->_parent;
				}*/
	...
	template<class K, class V, class KeyOfT, class Compare = less<K>>
	class RBTree
	{
	public:
		...
		typedef const RBTreeIterator<V, const V&, const V*> Const_Iterator;
		...
		pair<V, bool> Insert(const V& v)
		{
			if (_root == nullptr)
			{
				_root = new Node(v, Black);
				return pair<V, bool>(v, true);
				//const pair<K, V>& kv
			}
			...
			while (tmp)
			{
				if (tmp->_left != nullptr && KofT(v) < KofT(tmp->_date))
				{
					tmp = tmp->_left;
				}
				else if (tmp->_right != nullptr && KofT(tmp->_date) < KofT(v))
				{
					tmp = tmp->_right;
				}
				else
			...
			// 开始插入
			if (KofT(v) < KofT(tmp->_date))
			{
				tmp->_left = newnode;
			}
			else if (KofT(tmp->_date) < KofT(v))
			{
				tmp->_right = newnode;
			}
			else
		...
		Iterator Find(const K& key) const
		{
			...
			while (tmp)
			{
				if (less(key, KofT(tmp->_date)))
				{
					tmp = tmp->_left;
				}
				else if (less(KofT(tmp->_date), key))
				{
					tmp = tmp->_right;
				}
				else
	...
	private:
		...
		bool Check(Node* root, int blackNum, const int refNum)
		{
			...
			{
				//cout << root->_date << "存在连续的红色结点" << endl;
				return false;
			}

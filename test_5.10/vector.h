...
namespace QzQz
{
	...
	public:
		...
		vector(const vector<T>& val)
		{
			...
			_start = tmp;
			_finish = _end_of_storage = _start + val.size();
		}
		/*vector(vector<T> tmp)
		{
	...
	{
		//std::cout << vector<T>::xyz << std::endl;
	...
	void test_vector6()
	{
		...
		print_vector(v1);
		//std::cout << std::find(v1.begin(), v1.end(), 1) << std::endl;

		int x;
		std::cin >> x;

		//int* pos = std::find(v1.begin(), v1.end(), x);
		auto pos = std::find(v1.begin(), v1.end(), x);
